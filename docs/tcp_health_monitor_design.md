# Titan-DNS TCP 端口健康监控与 IP 动态离线过滤设计方案

本设计方案旨在为 `titan-dns` 增加周期性的 TCP 端口健康检测与动态离线 IP 过滤机制。
其核心功能为：**定时对所有配置 of POP IP 进行 TCP 端口连接测试，若测试失败则标记该 IP 离线，在 DNS 解析轮询时不返回该 IP；当 IP 恢复通畅时自动重新上线。**
...
2. **并发安全离线表**：使用 `sync.Map` 维护一个全局离线 IP 集合 `OfflineIPs`，提供 $O(1)$ 的并发读写，无锁竞争风险。
3. **原地无内存分配轮询**：在轮询 IP 时，利用现有的 Round-Robin 计数器作为偏移起点，通过取模运算在原数组上顺时针遍历，跳过离线 IP，零内存分配。
...
* **YAML 配置 (config.yaml)**
  ...
  monitor:
    ...
    unhealthy_threshold: 3 # 连续失败多少次后标记离线
...
### 2. 离线 IP 注册表与后台健康检测设计
新建 `monitor.go` 管理离线 IP 状态与并发 TCP 探测。我们将整个监控逻辑封装在独立的 `TCPMonitor` 结构中，防止污染 `DNSHandler`。
...
* **并发安全离线表 (OfflineIPs)**
  ...
  func (o *OfflineIPs) Add(ip string)    { o.ips.Store(ip, true) }
  func (o *OfflineIPs) Remove(ip string) { o.ips.Delete(ip) }
...
* **探测机制与启动**
  ...
  4. 当 IP 探测失败：
     - 累加失败计数 `Failures++`。
     - 当连续失败次数达到 `unhealthy_threshold` 时，将该 IP 加入 `offlineIPs` 注册表。
  5. 当 IP 探测成功：
     - 重置连续失败计数 `Failures = 0`。
     - **只要有一次成功，立刻**将该 IP 从 `offlineIPs` 移除，使其重新上线参与轮询。

---

### 3. DNS 轮询与离线 IP 过滤逻辑设计
...
func (lb *LoadBalancer) BalanceByRR(popID string, isOffline func(string) bool) (string, uint64) {
	...
	for i := uint64(0); i < n; i++ {
		...
		if !isOffline(ip) {
			return ip, index // 找到首个在线的可用 IP，直接返回
		}
	}

	// 3. 若所有 IP 都离线，根据约定返回空
	return "", 0
}
```

* **为何是零开销**：此算法遍历的上限为 `N`（POP 内 IP 数量），并且仅在原切片上通过下标偏移遍历，不进行任何切片复制、扩容或追加操作，内存分配为 0。同时把 `atomic.AddUint64` 放入循环中，可以在遇到离线 IP 时自动消耗和跳过对应的槽位。

---

### 4. 会话缓存（Sticky Cache）覆盖机制设计
`titan-dns` 拥有一个会话缓存 `StickyCache`，用于保持同一个 session 总是路由到相同的 IP。
如果该 IP 被标记为了离线，我们需要确保后续请求不再返回缓存中的故障 IP：
...
* 在 `handler.go` 的 `resolveSubdomain` 中：
  ...
  if ip, ok := h.cache.Get(name); ok {
      ...
      if !h.offlineIPs.Contains(ip) {
          return ip, 0 // 依然在线，直接返回缓存 IP
      }
      // 已离线，旁路缓存，重新走负载均衡轮询获取健康 IP
  }
...
* **Ref 别名**：`BalanceByRR` 会首先解析 `Ref` 指向的基准 POP 数据（`ipsData = nextData`）。因此当基准 POP 的 IP 离线时，引用别名的子 POP 在遍历时会自动感知该状态，无需任何同步逻辑。
* **Follow 关系**：跟随者关系会通过组合父 POP 的 IP 列表存入 `data.IPs` 中。监控探测会获取这批合并后的 IP。当其中某个父 POP 的 IP 离线时，跟随者 POP 在遍历自己的 `data.IPs` 时，遍历逻辑依然会在 `isOffline` 校验中将其过滤，逻辑完全自适应。

---

### 6. 管理与清理 API/CLI 命令设计

为了方便运维管理，我们在 `titan-dns` 中新增了清理离线 IP 状态与监控历史计数器的命令：

* **HTTP API 端点 (`/api/v1/offline/clear`)**：
  验证调用者签名后，清空离线 IP 注册表 (`offlineIPs`) 与 `TCPMonitor` 内部的历史探测次数计数器，恢复所有 IP 上线。
  ```go
...
* **CLI 命令 (`clear-offline`)**：
  可以通过运行 CLI 命令直接调用上述 API 强制重置状态：
  ```bash
  ./titan-dns -m cli -cmd clear-offline
  ```
...
4. 关闭 Mock 监听服务，验证经过间隔时间后该 IP 自动标记为离线，解析返回 `""`。
5. 重新启动 Mock 服务，验证 IP 自动重新上线并正常恢复解析。

### 手动验证
1. 启动 `titan-dns` 并配置两个测试 IP（如本地监听的 `127.0.0.1` 和未监听的 `127.0.0.2`，端口设为 `9000`）。
2. 在 `9000` 端口启动测试服务，测试 DNS 查询，确认轮询两个 IP。
3. 关掉测试服务，查看日志，应该输出：`[MONITOR] IP 127.0.0.1 has failed... Marking as offline.`。
4. 再次执行 `nslookup` 或 `dig`，确认在线的 IP 得以返回，而离线的 IP 不再返回。
