...
func TestGetAllTrafficPer5Min(t *testing.T) {
	conf := redis.RedisConf{Host: "127.0.0.1:6379", Type: "node"}
	rd := redis.MustNewRedis(conf)

	l := GetAllStats5MinLogic{ctx: context.Background(), svcCtx: &svc.ServiceContext{Redis: rd}}
	resp, err := l.GetAllStats5Min(&pb.AllStatsReq{Hours: 24})
	if err != nil {
