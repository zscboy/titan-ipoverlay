...
func TestGetUserTrafficPer5Min(t *testing.T) {
	conf := redis.RedisConf{Host: "127.0.0.1:6379", Type: "node"}
	rd := redis.MustNewRedis(conf)

	l := GetUserStats5MinLogic{ctx: context.Background(), svcCtx: &svc.ServiceContext{Redis: rd}}
	resp, err := l.GetUserStats5Min(&pb.UserStatsReq{Username: "test1", Hours: 24})
	if err != nil {
