...
type GetUserStatsDayLogic struct {
	ctx    context.Context
	svcCtx *svc.ServiceContext
	logx.Logger
}

func NewGetUserStatsDayLogic(ctx context.Context, svcCtx *svc.ServiceContext) *GetUserStatsDayLogic {
	return &GetUserStatsDayLogic{
		ctx:    ctx,
		svcCtx: svcCtx,
...
func (l *GetUserStatsDayLogic) GetUserStatsDay(in *pb.UserStatsDayReq) (*pb.UserStatsResp, error) {
	if in.Days == 0 {
		return nil, fmt.Errorf("request hours")
	}

	traffics, err := model.ListUserTrafficPerDay(l.ctx, l.svcCtx.Redis, in.Username, int(in.Days))
	if err != nil {
		return nil, err
	}
